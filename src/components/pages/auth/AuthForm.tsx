...
const AuthForm: FC<AuthFormProps> = ({ handleDataSubmit }) => {
  ...
  const { handleChange, values, touched, errors, handleSubmit,setFieldValue, resetForm, isSubmitting } =
    useFormik({
      initialValues: {
        email: '',
        password: '',
      ...
      >
        ...
        {pathname === '/auth/register' && (
          <div className="w-full space-y-3">
            <div>
              <input defaultChecked onClick={(e:any)=>setFieldValue('group',e.target.value)} type="radio" name="group" id="candidate" value={"CANDIDATE"} />
              <label htmlFor="candidate">Join As Candidate</label>
            </div>
            <div>
              <input onClick={(e:any)=>setFieldValue('group',e.target.value)} type="radio" name="group" id="company" value={"COMPANY"} />
